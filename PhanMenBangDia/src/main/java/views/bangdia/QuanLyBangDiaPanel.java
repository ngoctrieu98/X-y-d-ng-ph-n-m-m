...
public class QuanLyBangDiaPanel extends JPanel {
	private static final long serialVersionUID = 1L;
	private JTextField ma_txt, phiTreHan_txt, phiThue_txt, soNgayDuocThue_txt;
	...
	private BangDiaImpl dao = new BangDiaImpl();

	private TieuDeImpl tieuDeDao = new TieuDeImpl();

	@SuppressWarnings({ "rawtypes", "unchecked", "serial" })
	public QuanLyBangDiaPanel() {
		...
		JLabel lblNewLabel_3 = new JLabel("Quản lý băng đĩa");
...
//		loaiBox.setEnabled(false);
		...
		loaiBox.setModel(new DefaultComboBoxModel(tuaDia));
		...
		JLabel lblNewLabel_1 = new JLabel("Phí thuê: ");
		...
		JLabel lblNewLabel_1_3 = new JLabel("Phí trễ hạn: ");
		...
		JLabel lblNewLabel_1_3_1 = new JLabel("Số ngày được thuê: ");
		lblNewLabel_1_3_1.setPreferredSize(new Dimension(120, 25));
		panel_2_2_1.add(lblNewLabel_1_3_1);

		soNgayDuocThue_txt = new JTextField();
		soNgayDuocThue_txt.setPreferredSize(new Dimension(100, 25));
		panel_2_2_1.add(soNgayDuocThue_txt);
		...
		model = new DefaultTableModel(new Object[][] {}, new String[] { "Mã đĩa", "Tên tiêu đề", "Phí thuê",
				"Phí trễ hạn", "Số ngày được thuê", "Tình trạng" }) {
		...
		table.addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent arg0) {
				...
				if (table.getSelectedRow() == -1) {
					...
					soNgayDuocThue_txt.setText(table.getValueAt(table.getSelectedRow(), 4).toString());

					phiThue_txt.setEnabled(true);
					phiTreHan_txt.setEnabled(true);
					soNgayDuocThue_txt.setEnabled(true);
//					loaiBox.setEnabled(true);

					loaiBox.setSelectedItem(table.getValueAt(table.getSelectedRow(), 1).toString());
				...
				if(MainScreen.checkLogin == MainScreen.logout) {
					...
					if (!check) {
						...
						for (int i = 0; i < Integer.parseInt(dulieu); i++) {
							...
							System.out.println(dao.them(bangDia));
						}
						addTieuDe_btn.setText("Thêm");
	...
	public void loadDataToTable() {
		...
		for (BangDia bangDia : list) {
			...
			if (bangDia.getTinhTrang() == BangDia.DA_DAT)
				tinhTrang = "Đã đặt";
			else if (bangDia.getTinhTrang() == BangDia.DIA_DANG_THUE)
				tinhTrang = "Đang thuê";
			else if (bangDia.getTinhTrang() == BangDia.TRONG_KHO)
				tinhTrang = "Đang trên kệ";
			Object[] o = new String[] { bangDia.getMa() + "", bangDia.getMaTieuDe().getTen(), bangDia.getPhiThue() + "",
					bangDia.getPhiTreHan() + "", bangDia.getSoNgayDuocThue() + "", tinhTrang };
			model.addRow(o);
		}
	}
	...
	public void backToDefault() {
		...
		soNgayDuocThue_txt.setEnabled(false);
		loaiBox.setEnabled(false);
